...
function generateRelativeNumbers(yAxisKey, denominator) {
  ...
  data.forEach((element) => {
    element[yAxisKey + "by" + denominator] =
      (parseFloat(element[yAxisKey]) / element[denominator]) * multiplier;
  });
...
function generateLineChart(yAxisKey, elementID, denominator = "needIndex") {
  ...
  tdata = trenddata.filter((x) => !isNaN(x[yAxisKey]));
  ...
  return new Chart(document.getElementById(elementID + "-line"), config);
}
...
function generateBarChart(yAxisKey, PRAC_CODE, elementID, denominator) {
  ...
  tdata = data.filter((x) => !isNaN(x[yAxisKey]));
  ...
  return new Chart(document.getElementById(elementID + "-bar"), config);
}
...
function generateCard(
  ...
  html =
    ...
    `</h7><br>
        ...
        <div class="card-body">
        <h7>Comparison to other organisations</h7>
            <div class="row">
                <div class="col">
